...
<div class="messagehub-shell">
	<h1><?php echo $e($t('title', 'Message queue')); ?></h1>
	<p><?php echo $e($t('lead', 'Queued, retrying, processing and failed messages.')); ?></p>
...
<script type="module">
	...
	const TRANSPORT_FILTER_OPTIONS = [{ value: '', label: tr('all_transports', 'All transports') }, ...<?php echo json_encode($transportFilterOptions, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>];
	const TYPE_FILTER_OPTIONS = [{ value: '', label: tr('all_types', 'All types') }, ...<?php echo json_encode($typeFilterOptions, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES); ?>];
	let grid = null;
	...
	function setLog(message) { const element = document.querySelector('#messagehub-queue-output'); if(!element) { return; } element.replaceChildren(); const label = document.createElement('strong'); label.textContent = tr('last_action', 'Last action:'); element.appendChild(label); element.appendChild(document.createTextNode(' ' + getText(message, tr('none_label', 'None')))); }
	function statusLabel(value) { const key = getText(value, '').toLowerCase(); return ({ queued: tr('status_queued', 'Queued'), retry_wait: tr('status_retry_wait', 'Retry wait'), processing: tr('status_processing', 'Processing'), sent: tr('status_sent', 'Sent'), failed: tr('status_failed', 'Failed'), cancelled: tr('status_cancelled', 'Cancelled') })[key] || getText(value); }
	function statusPill(value) { const pill = document.createElement('span'); pill.className = 'messagehub-pill messagehub-pill-' + getText(value, '').replace(/[^a-z0-9_-]/gi, '-'); pill.textContent = statusLabel(value); return pill; }
			...
			layoutContributions() {
				return [{
					...
					render() {
						...
						processButton.textContent = tr('process_batch', 'Process batch');
						processButton.addEventListener('click', async () => {
							const response = await postJson({ mode: 'process', limit: 20 });
							await refreshGrid();
							setLog(tr('processed_queued_messages', 'Processed {count} queued messages.', { count: response.processed || 0 }));
						});
	...
	grid = new ModularGrid('#messagehub-queue-grid', { layout, adapter, strings: GRID_STRINGS, dataMode: 'server', server: { searchDebounceMs: 220, watchStateKeys: ['query', 'filters'] }, features: { paging: false }, plugins: [createQueueActionsPlugin(), SearchPlugin, FiltersPlugin, HeaderMenuPlugin, InfoPlugin, RowActionsPlugin, ResetPlugin, SessionStoragePlugin, InfiniteScrollPlugin], pluginOptions: { search: { zone: 'topLine1', order: 10, label: tr('search', 'Search'), placeholder: tr('queue_search_placeholder', 'Search id, type, subject or error') }, filters: { zone: 'topLine2', order: 10, stateKey: 'filters', showClearButton: true, clearLabel: tr('clear_filters', 'Clear filters'), fields: [{ key: 'status', label: tr('status', 'Status'), type: 'select', options: [{ value: '', label: tr('all_statuses', 'All statuses') }, { value: 'queued', label: tr('status_queued', 'Queued') }, { value: 'retry_wait', label: tr('status_retry_wait', 'Retry wait') }, { value: 'processing', label: tr('status_processing', 'Processing') }, { value: 'sent', label: tr('status_sent', 'Sent') }, { value: 'failed', label: tr('status_failed', 'Failed') }, { value: 'cancelled', label: tr('status_cancelled', 'Cancelled') }] }, { key: 'transport_name_exact', label: tr('transport', 'Transport'), type: 'select', options: TRANSPORT_FILTER_OPTIONS }, { key: 'type_name_exact', label: tr('type', 'Type'), type: 'select', options: TYPE_FILTER_OPTIONS }] }, reset: { zone: 'topLine1', order: 20, label: tr('reset', 'Reset'), sections: ['query', 'filters', 'columns'] }, sessionStorage: { key: 'messagehub-queue-grid', sections: ['query', 'filters', 'columns'] }, info: { zone: 'statusZone', order: 10, displayMode: 'loaded' }, infiniteScroll: { threshold: 180, pageSize: 50, containerSelector: '.mg-table-scroll' }, rowActions: { items: [{ key: 'cancel', label: tr('cancel', 'Cancel'), async onClick(context) { await postJson({ mode: 'cancel', id: context.row.id }); await refreshGrid(); setLog(tr('cancelled_item', 'Cancelled {id}.', { id: context.row.id })); } }] } }, columns: [{ key: 'created_at', label: tr('created', 'Created'), width: 170 }, { key: 'status', label: tr('status', 'Status'), width: 120, render: statusPill }, { key: 'type_name', label: tr('type', 'Type'), width: 220 }, { key: 'subject', label: tr('subject', 'Subject'), width: 360 }, { key: 'transport_name', label: tr('transport', 'Transport'), width: 140 }, { key: 'attempts', label: tr('attempts', 'Attempts'), width: 100 }, { key: 'last_error', label: tr('last_error', 'Last error'), width: 360 }, { key: 'id', label: tr('id', 'ID'), width: 260, visible: false }] });
	await grid.init();
	setLog(tr('queue_loaded', 'Queue loaded.'));
</script>
