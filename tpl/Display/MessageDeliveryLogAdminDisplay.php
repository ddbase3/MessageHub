...
<div class="messagehub-shell">
	<h1>Message deliveries</h1>
	<p>Delivery attempts, results and transport errors.</p>
	<div class="messagehub-grid"><div id="messagehub-delivery-grid"></div><div id="messagehub-delivery-output" class="messagehub-output"></div></div>
</div>
<script type="module">
	...
	const ENDPOINT_URL = <?php echo json_encode((string)$this->_['service'], JSON_UNESCAPED_SLASHES); ?>; let grid = null;
	function setLog(text) { const el = document.querySelector('#messagehub-delivery-output'); if(el) el.textContent = text || ''; }
	...
	grid = new ModularGrid('#messagehub-delivery-grid', { layout, adapter, dataMode:'server', features:{paging:true}, plugins:[SearchPlugin, FiltersPlugin, HeaderMenuPlugin, InfoPlugin, RowActionsPlugin, ResetPlugin, SessionStoragePlugin], pluginOptions:{ search:{zone:'topLine1', order:10, label:'Search', placeholder:'Search id, queue, type, subject or error'}, filters:{zone:'topLine2', order:10, stateKey:'filters', showClearButton:true, fields:[{key:'status', label:'Status', type:'select', options:[{value:'', label:'All statuses'}, {value:'processing', label:'Processing'}, {value:'sent', label:'Sent'}, {value:'failed', label:'Failed'}]}, {key:'transport_name', label:'Transport', type:'text'}, {key:'type_name', label:'Type', type:'text'}]}, reset:{zone:'topLine1', order:20, label:'Reset', sections:['query','filters','columns']}, sessionStorage:{key:'messagehub-delivery-grid', sections:['query','filters','columns']}, info:{zone:'statusZone', order:10, displayMode:'loaded'}, rowActions:{items:[{key:'detail', label:'Show detail', async onClick(ctx){ const r = await postJson({mode:'detail', id:ctx.row.id}); setLog(JSON.stringify(r.detail || {}, null, 2)); }}]} }, columns:[{key:'created_at', label:'Created', width:170}, {key:'status', label:'Status', width:120, render: statusPill}, {key:'type_name', label:'Type', width:220}, {key:'subject', label:'Subject', width:360}, {key:'transport_name', label:'Transport', width:140}, {key:'attempts', label:'Attempts', width:90}, {key:'error_message', label:'Error', width:380}, {key:'id', label:'ID', width:260, visible:false}] });
	await grid.init(); setLog('Deliveries loaded.');
</script>
