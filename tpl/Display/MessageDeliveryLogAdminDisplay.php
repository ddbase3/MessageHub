...
<script type="module">
	...
	function statusLabel(value) { const key = getText(value, '').toLowerCase(); return ({ processing: tr('status_processing', 'Processing'), sent: tr('status_sent', 'Sent'), failed: tr('status_failed', 'Failed') })[key] || getText(value); }
	...
	grid = new ModularGrid('#messagehub-delivery-grid', { layout, adapter, strings: GRID_STRINGS, dataMode: 'server', server: { searchDebounceMs: 220, watchStateKeys: ['query', 'filters'] }, features: { paging: false }, plugins: [SearchPlugin, FiltersPlugin, HeaderMenuPlugin, InfoPlugin, RowActionsPlugin, ResetPlugin, SessionStoragePlugin, InfiniteScrollPlugin], pluginOptions: { search: { zone: 'topLine1', order: 10, label: tr('search', 'Search'), placeholder: tr('delivery_search_placeholder', 'Search id, queue, type, subject or error') }, filters: { zone: 'topLine2', order: 10, stateKey: 'filters', showClearButton: true, clearLabel: tr('clear_filters', 'Clear filters'), fields: [{ key: 'status', label: tr('status', 'Status'), type: 'select', options: [{ value: '', label: tr('all_statuses', 'All statuses') }, { value: 'processing', label: tr('status_processing', 'Processing') }, { value: 'sent', label: tr('status_sent', 'Sent') }, { value: 'failed', label: tr('status_failed', 'Failed') }] }, { key: 'transport_name_exact', label: tr('transport', 'Transport'), type: 'select', options: TRANSPORT_FILTER_OPTIONS }, { key: 'type_name_exact', label: tr('type', 'Type'), type: 'select', options: TYPE_FILTER_OPTIONS }] }, reset: { zone: 'topLine1', order: 20, label: tr('reset', 'Reset'), sections: ['query', 'filters', 'columns'] }, sessionStorage: { key: 'messagehub-delivery-grid', sections: ['query', 'filters', 'columns'] }, info: { zone: 'statusZone', order: 10, displayMode: 'loaded' }, infiniteScroll: { threshold: 180, pageSize: 50, containerSelector: '.mg-table-scroll' }, rowActions: { items: [{ key: 'detail', label: tr('show_detail', 'Show detail'), async onClick(context) { const response = await postJson({ mode: 'detail', id: context.row.id }); setLog(JSON.stringify(response.detail || {}, null, 2)); } }] } }, columns: [{ key: 'created_at', label: tr('created', 'Created'), width: 170 }, { key: 'status', label: tr('status', 'Status'), width: 120, render: statusPill }, { key: 'type_name', label: tr('type', 'Type'), width: 220 }, { key: 'subject', label: tr('subject', 'Subject'), width: 360 }, { key: 'transport_name', label: tr('transport', 'Transport'), width: 140 }, { key: 'attempts', label: tr('attempts', 'Attempts'), width: 90 }, { key: 'error_message', label: tr('error', 'Error'), width: 380 }, { key: 'id', label: tr('id', 'ID'), width: 260, visible: false }] });
	await grid.init();
	setLog(tr('deliveries_loaded', 'Deliveries loaded.'));
</script>
